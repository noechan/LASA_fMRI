...
ses=['_3'];
% Subjects fnames
subjects_s3_n19 = readtable(fullfile(code_path, 'subjects_s3_n19.csv'));
...
for sub=1:size(subjects_s3_n19,1)
    ...
    sub_path=fullfile(data_path,cohort,prep,song, subjects_s3_n19{sub,2}, [subjects_s3_n19{sub,2} ses]);
    if exist(sub_path)==7
        cd(sub_path)
        if exist(['baseline_false_' song '_' subjects_s3_n19{sub,2} '.mat'])==2
            load(fullfile(sub_path,(['baseline_false_' song '_' subjects_s3_n19{sub,2}])))
            baseline_false_sub{c1,1}=subjects_s3_n19{sub,2};
            baseline_false_sub{c1,2}=numel(baseline_false);
            c1=c1+1; 
        end
       
        if exist(['listen_false_' song '_' subjects_s3_n19{sub,2} '.mat'])==2
            load(fullfile(sub_path,(['listen_false_' song '_' subjects_s3_n19{sub,2}])))
            listen_false_sub{c2,1}=subjects_s3_n19{sub,2};
            listen_false_sub{c2,2}=numel(listen_false);
            c2=c2+1; 
        end
        
        if exist(['sing_along_false_' song '_' subjects_s3_n19{sub,2} '.mat'])==2
            load(fullfile(sub_path,(['sing_along_false_' song '_' subjects_s3_n19{sub,2}])))
            sing_along_false_sub{c3,1}=subjects_s3_n19{sub,2};
            sing_along_false_sub{c3,2}=numel(sing_along_false);
            c3=c3+1; 
        end
        
        if exist(['sing_memo_false_' song '_' subjects_s3_n19{sub,2} '.mat'])==2
            load(fullfile(sub_path,(['sing_memo_false_' song '_' subjects_s3_n19{sub,2}])))
            sing_memo_false_sub{c4,1}=subjects_s3_n19{sub,2};
...
save(fullfile(code_path, ['patients_s3_n19_false_trials_sub_' song ses '.mat']), 'baseline_false_sub', 'listen_false_sub', 'sing_along_false_sub', 'sing_memo_false_sub')

%% Create plots
cd(code_path)
song=['Uulaa'];
ses=['_3'];
T=load(['patients_s3_n19_false_trials_sub_' song ses '.mat']);
...
save(['patients_s3_n19_Trials_summary_' song '_ses' ses '.mat'], 'baseline_sum','listen_sum','sing_along_sum','sing_memo_sum')
...
savefig(gcf,['Patients Ses3 N19 False Trials All' song '_ses' ses '.fig'])
...
savefig(gcf,['Patients Ses3 N19 Mean False Trials' song '_ses' ses '.fig'])
close all

% plot mean performance by song and trial
...
% plot overall mean performance by song and trial

clear all
ses=['_3'];
Tydyy=load(['patients_s3_n19_false_trials_sub_Tydyy' ses '.mat']);
Uulaa=load(['patients_s3_n19_false_trials_sub_Uulaa' ses '.mat']);

Tydyy_sum=load(['patients_s3_n19_Trials_summary_Tydyy_ses' ses '.mat']);
Uulaa_sum=load(['patients_s3_n19_Trials_summary_Uulaa_ses' ses '.mat']);
...
savefig(gcf,['Patients Ses3 N19 TU Singing Performance_ses' ses '.fig'])
